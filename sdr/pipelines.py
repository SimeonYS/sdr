...
class SdrPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `sdr`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM sdr WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO sdr (date, title, link, content)"
